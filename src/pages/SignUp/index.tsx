...
const SignUp : React.FC<IProps> = ({ navigation }):JSX.Element => {
  ...
  const onUserFieldChange = useCallback((field: UserProps) => {
    setFormFields((fields) => (
      {
        ...fields,
        user: {
  ...
  const createUser = async(email: string, password: string) => {
    try {
      ...
      if (error.code === 'auth/email-already-in-use') {
        alert.error('O email informado já esta em uso, por favor informe outro email')
      }
      ...
      if (error.code === 'auth/invalid-email') {
        ...
        alert.error('Ocorreu um erro ao efetuar cadastro, por favor tente novamente')
      }
  ...
  const onSubmit = useCallback(async () => {
    ...
    if(!name || !email || !phone || !birthDate || !password || !passwordConfirmation) {
    ...
    .catch((_error) => {
      alert.error('Erro ao realizar cadastro, por favor tente novamente em alguns instantes')
    })
    .finally(() => {
      setIsLoading(false)
      setFormFields({})
      ...
      >
        ...
        <S.FormContainer>
          <S.InputText
            label="Nome"
            placeholder="Nome"
            onChange={(e: any) => onCustomerFieldChange({ name: e.nativeEvent.text })}
          ...
          <S.InputText
            ...
            autoCapitalize="none"
            onChange={(e: any) => onCustomerFieldChange({ email: e.nativeEvent.text })}
          ...
          {isNotAdult && (
            <>
              <S.InputText
                ...
                errorDescription={EMPTY_FIELDS_MESSAGE}
                onChange={(e: any) => onCustomerFieldChange({ parentName: e.nativeEvent.text })}
              />
              <S.InputText
                ...
                onChangeText={(e: any) => onCustomerFieldChange({parentCPF: e})}
              />
            </>
          )}
          ...
          <S.InputText
            ...
            type="password"
            autoCapitalize="none"
            hasError={!formFields?.user?.password && isEmptyFields || isPasswordsNotMatch}
            errorDescription={isEmptyFields ? EMPTY_FIELDS_MESSAGE : PASSWORDS_NOT_MATCH}
          ...
          <S.InputText
            ...
            type="password"
            autoCapitalize="none"
            hasError={!formFields?.user?.passwordConfirmation && isEmptyFields || isPasswordsNotMatch}
            errorDescription={isEmptyFields ? EMPTY_FIELDS_MESSAGE : PASSWORDS_NOT_MATCH}
            onChange={(e: any) => onUserFieldChange({ passwordConfirmation: e.nativeEvent.text })}
